...
def extract_products(html: str, soup: BeautifulSoup | None = None, base_url: str = "") -> list:
    ...
    for script in soup.find_all("script", type="application/ld+json"):
        try:
            ...
            logger = get_logger("product_extractor")
            logger.debug(f"JSON-LD product extraction failed: {e}")
    ...
    if not found:
        ...
        for container in soup.find_all(
            ...
            and tag.get("class")
            and any("product" in c.lower() or "item" in c.lower() for c in tag.get("class"))
        ):
            name_tag = container.find(["h1", "h2", "h3"])
            price_tag = container.find(
                lambda t: t.name in ["span", "div", "p"]
                and t.get("class")
                and any("price" in c.lower() for c in t.get("class"))
            )
            ...
            if name_tag:
                ...
                if name and not any(p["name"] == name for p in found):
                    found.append({"name": name, "price": price, "description": "", "url": link})
